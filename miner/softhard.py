...
class BatchMiner():
    ...
    def __call__(self, batch, labels, return_distances=False, train_x=None, step=0):
        ...
        if self.args.save_triplet:
            torch.save(train_x.detach().cpu(), os.path.join(self.args.dir_softhardtriplets, str(step)+'_x.pth'))
            np.save(os.path.join(self.args.dir_softhardtriplets, str(step)+'.npy'), np.stack((anchors, positives, negatives)))
