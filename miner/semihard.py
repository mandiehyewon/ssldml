...
class BatchMiner():
    def __init__(self, args):
        self.args         = args
        self.name         = 'semihard'
        self.margin       = args.loss_triplet_margin

    def __call__(self, batch, labels, return_distances=False, train_x=None, step=0):
        ...
        if self.args.save_triplet:
            torch.save(train_x.detach().cpu(), os.path.join(self.args.dir_semihardtriplets, str(step)+'_x.pth'))
            np.save(os.path.join(self.args.dir_semihardtriplets, str(step)+'.npy'), np.stack((anchors, positives, negatives)))
